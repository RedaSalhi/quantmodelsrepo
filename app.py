...
└── hull_white/
    ...
    n_paths = st.sidebar.slider("Number of simulation paths", 10, 1000, 100, 10,
                               help="More paths give smoother results but take longer to compute")
    T = st.sidebar.slider("Time horizon (years)", 0.1, 5.0, 1.0, 0.1,
                         help="How far into the future to simulate")
    
    if model_choice == "Black-Scholes SDE":
        ...
        with col1:
            ...
            spot = st.number_input("Initial stock price ($)", value=100.0, min_value=1.0, step=1.0,
                                 help="The current market price of the stock")
            mu = st.slider("Drift rate (μ)", -0.5, 0.5, 0.05, 0.01,
                          help="Expected annual return - higher values mean faster growth")
            sigma = st.slider("Volatility (σ)", 0.01, 1.0, 0.2, 0.01,
                            help="Annual volatility - higher values mean more randomness")
        ...
        with col1:
            ...
            S_max = st.number_input("Maximum asset price for grid", value=200.0, min_value=50.0, step=10.0,
                                  help="Upper boundary for the numerical grid")
            ...
            T_pde = st.slider("Time to maturity", 0.1, 2.0, 0.25, 0.05,
                            help="How much time until the option expires")
            r = st.slider("Risk-free interest rate", 0.0, 0.2, 0.05, 0.001,
                         help="The prevailing risk-free rate")
            ...
            M = st.slider("Price grid points", 50, 200, 100, 10,
                         help="More points = higher accuracy but slower computation")
        ...
        with col1:
            ...
            mu = st.slider("Stock drift rate", -0.5, 0.5, 0.05, 0.01,
                          help="Expected return of the stock")
            
            st.markdown("**Volatility Process:**")
            v0 = st.slider("Initial variance", 0.01, 1.0, 0.04, 0.01,
                          help="Starting level of volatility squared")
            kappa = st.slider("Mean reversion speed", 0.1, 10.0, 2.0, 0.1,
                            help="How quickly volatility reverts to its long-term mean")
            theta = st.slider("Long-term variance", 0.01, 1.0, 0.04, 0.01,
                            help="The long-term average level of variance")
            xi = st.slider("Volatility of volatility", 0.01, 2.0, 0.3, 0.01,
                          help="How much the volatility itself fluctuates")
            rho = st.slider("Correlation", -1.0, 1.0, -0.7, 0.1,
                           help="Correlation between stock returns and volatility changes")
            
            dt_choice = st.selectbox("Time step", [1/252, 1/52], index=0, 
                                   format_func=lambda x: f"Daily" if x == 1/252 else f"Weekly")
            ...
            try:
                with st.spinner("Simulating correlated stock and volatility paths..."):
                    ...
                    with col2:
                        ...
                        # Stock price paths (show subset for clarity)
        ...
        with col1:
            ...
            F0 = st.number_input("Initial forward rate", value=0.05, min_value=0.001, max_value=0.5, 
                                ...
                                help="Starting level of the forward interest rate")
            alpha = st.slider("Initial volatility (α)", 0.01, 1.0, 0.2, 0.01,
                            help="Starting level of volatility")
            beta = st.slider("Elasticity parameter (β)", 0.0, 1.0, 0.5, 0.1,
                            help="Controls how volatility depends on the forward rate level")
            rho = st.slider("Correlation (ρ)", -1.0, 1.0, -0.3, 0.1,
                           help="Correlation between forward rate and volatility")
            nu = st.slider("Volatility of volatility (ν)", 0.01, 2.0, 0.3, 0.01,
                          help="How much the volatility itself varies")
            ...
            try:
                with st.spinner("Simulating SABR forward rate and volatility dynamics..."):
                    ...
                    with col2:
                        ...
                        for i in range(sample_size):
                            fig.add_trace(go.Scatter(
                                x=time_steps, y=F_paths[:, i] * 100,  # Convert to percentage
                ...
                with analysis_col1:
                    final_forwards = F_paths[-1, :] * 100  # Convert to percentage
        ...
        with col1:
            ...
            mu = st.slider("Drift rate", -0.5, 0.5, 0.05, 0.01,
                          help="Expected return in the absence of jumps")
            sigma = st.slider("Diffusion volatility", 0.01, 1.0, 0.2, 0.01,
                            help="Continuous volatility component")
            ...
            mu_j = st.slider("Average jump size", -0.5, 0.5, -0.1, 0.01,
                           help="Expected logarithmic jump size (negative = downward bias)")
            sigma_j = st.slider("Jump size volatility", 0.01, 1.0, 0.3, 0.01,
                              help="Variability in jump sizes")
        ...
        if run_jump:
            try:
                ...
                with jump_col1:
                    ...
                    # Simple jump detection: returns > 2 standard deviations
                    return_std = np.std(returns)
                    jump_threshold = 3 * return_std
                    
                    large_moves = np.abs(returns) > jump_threshold
                    jump_frequency = np.sum(large_moves) / (len(time_steps) - 1) / n_paths
                ...
                with jump_col2:
                    ...
                    bs_model = BlackScholesSDE(spot, mu + lamb * mu_j, sigma, dt_choice)  # Drift adjustment
        ...
        with col1:
            ...
            if volatility_type == "Constant":
                ...
                vol_surface = lambda S, t: np.full_like(S, base_vol)
                ...
                time_decay = st.slider("Time decay factor", -2.0, 2.0, -0.5, 0.1,
                                     help="Negative = volatility decreases over time")
                ...
                level_slope = st.slider("Level sensitivity", -2.0, 2.0, 0.5, 0.1,
                                      help="How volatility changes with stock price level")
        ...
        if run_local_vol:
            try:
                ...
                with surface_col1:
                    ...
                    # Ensure vol_surface can handle arrays
        ...
        with col1:
            st.subheader("Model Parameters")
            r0 = st.slider("Initial interest rate", 0.0, 0.15, 0.05, 0.001,
                          help="Current short-term interest rate", format="%.3f")
            a = st.slider("Mean reversion speed", 0.1, 5.0, 1.0, 0.1,
                         help="How quickly rates revert to long-term mean")
            sigma = st.slider("Interest rate volatility", 0.001, 0.1, 0.02, 0.001,
                            help="Volatility of interest rate changes", format="%.3f")
            theta = st.slider("Long-term rate level", 0.0, 0.15, 0.05, 0.001,
                            help="Long-term average interest rate", format="%.3f")
